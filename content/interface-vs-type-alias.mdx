...
현대 웹 개발을 하다 보면 [TypeScript](https://www.typescriptlang.org/)로 작성된 코드를 많이 접하게 되는데, TypeScript는 컴파일 타임에 정적 타입 체크를 해주는 JavaScript의 상위 집합(superset) 언어입니다.
...
type alias에선 `&`를 이용하여 타입을 교차(intersection)시켜 확장된 새로운 프로퍼티를 추가합니다. 개인적으로 처음 intersection을 접할 땐 앞서 소개된 방식들보다 직관적이지 않다고 느꼈었는데, 이유는 TypeScript가 **집합**을 기반으로 동작하는 언어임을 받아들이지 못했었기 때문입니다.
...
여기서 좀 더 나아가 `Person`과 이름이 있는 컴퓨터 객체 집합을 교차시켜 인조인간 객체 타입 `Android`을 만들어보겠습니다.
...
이전의 예시와 다른 점은 name이라는 중복된 프로퍼티가 있습니다. `interface`와 마찬가지로 각 프로퍼티가 같은 타입일 땐 문제가 되지 않지만 만약 컴퓨터의 name이 string이 아니라면 어떻게 될까요?
...
으로 파생된 두 집합의 교집합은 원소가 없는 [공집합](https://ko.wikipedia.org/wiki/%EA%B3%B5%EC%A7%91%ED%95%A9)이기 때문에 name은 never 타입이 되어 어떠한 값도 할당할 수 없게 됩니다. 따라서 intersection은 단순히 프로퍼티를 추가한다가 아닌 교집합을 생성한다는 것을 기억해야 합니다.
...
최근엔 아래와 같은 이유로 interface를 잘 사용하지 않게 되었습니다.

- `type`이 더 짧음
- 재사용되지 않는 타입은 주로 인라인으로 만들고 `&`와 `|`([union](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#union-types))를 바로 사용
- 전부 type alias로 사용하여 통일성 유지
  - [우아한형제들 기술블로그](https://techblog.woowahan.com/9804/#toc-5) 결론 참고

interface와 type alias를 모두 써보면서 느낀 점은 개인 및 팀의 컨벤션에 따라 정하기 나름이고, 각 키워드가 필요한 경우가 생기기 때문에 상황에 맞춰 사용하면 된다는 것입니다. 그리고 이번 글에서 소개되진 않았지만 TypeScript의 [Structural Type System](https://www.typescriptlang.org/docs/handbook/typescript-in-5-minutes.html#structural-type-system) 및 [Freshness](https://radlohead.gitbook.io/typescript-deep-dive/type-system/freshness)도 함께 알아보시길 추천드리며 글을 마칩니다. 🙆‍♂️
