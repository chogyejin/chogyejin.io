...
3. staged된 파일이 있어 diff가 true가 아닌 값으로 종료되면,()
   ...
   3. staged된 파일이 있어 diff가 0이 아닌 상태로 종료되면 commit & push, 그게 아니고 staged된 파일이 없으면 workflow 콘솔에 로그(`echo`)만 남김 ([참고](https://stackoverflow.com/a/28772375/17685810))
